...
if check_sys rhel; then
    ...
    sed -i "s@^pm.max_children.*@pm.max_children = 5@" "${php_conf}"
    sed -i "s@^pm.start_servers.*@pm.start_servers = 1@" "${php_conf}"
    sed -i "s@^pm.min_spare_servers.*@pm.min_spare_servers = 1@" "${php_conf}"
    sed -i "s@^pm.max_spare_servers.*@pm.max_spare_servers = 1@" "${php_conf}"
